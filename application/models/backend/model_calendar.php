...
Class Model_calendar extends CI_Model
{
    ...
    public function SelectByX($param = null, $keyword = '', $orderby = '`ID` DESC', $page = 0, $perpage = null)
    {
        if ($perpage != null) {
            if (is_array($param))
                return $this->db->or_like(array('title' => $keyword, 'content' => $keyword))->where($param)->order_by($orderby)->limit($perpage, $page)->get(self::DB_TABLE)->result_array();
            else
                return $this->db->or_like(array('title' => $keyword, 'content' => $keyword))->order_by($orderby)->limit($perpage, $page)->get(self::DB_TABLE)->result_array();
        } else {
            if (is_array($param)) {
                return $this->db->or_like(array('title' => $keyword, 'content' => $keyword))->where($param)->order_by($orderby)->get(self::DB_TABLE)->result_array();
            } else {
                return $this->db->or_like(array('title' => $keyword, 'content' => $keyword))->order_by($orderby)->get(self::DB_TABLE)->result_array();
            }
        }
    ...
    {
        if (is_array($param))
            return $this->db->or_like(array('title' => $keyword, 'content' => $keyword))->where($param)->get(self::DB_TABLE)->num_rows();
        else
            return $this->db->or_like(array('title' => $keyword, 'content' => $keyword))->get(self::DB_TABLE)->num_rows();
    }
     ...
     **/
    public function InsertNewItem($data = null)
    {
        $this->populate($data);
